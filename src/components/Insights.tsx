...
export const Insights = ({ userId }: InsightsProps) => {
    ...
    if (loading) {
        return (
            <View className="bg-card rounded-[32px] p-6 shadow-xl mb-8"
    ...
    return (
        <View className="mb-8">
            <TouchableOpacity 
                className="bg-card rounded-[32px] p-6 shadow-xl"
                style={{ 
                            ...
                            <View>
                                ...
                                >
                                    {data.topWords.map((item) => (
                                        ...
                                        >
                                            <Text className="font-q-semibold text-sm" style={{ color: currentAccent.hex }}>
                                                {item.word} <Text className="text-xs" style={{ color: `${currentAccent.hex}99` }}>×{item.count}</Text>
                                            </Text>
                                        </View>
                                    ))}
                                </ScrollView>
                        ...
                        <View className="w-full max-w-[220px] mb-6">
                            ...
                            <View className="w-full h-2 bg-inactive/10 rounded-full overflow-hidden">
                                <View 
                                    ...
                                    style={{ 
                                        width: `${Math.min(((profile?.max_streak || 0) / 7) * 100, 100)}%`,
